...
class SM(kernels.SM):
    ...
    def eKxz(self, Z, Xmu, Xcov):
        ...
        S_inv_ex = self._expand_and_tile(S_inv, 4, 1, Q) # N Q P P

        A_q = tf.matrix_inverse(S_inv_ex + Sigma_q_inv_ex) # N Q P P
        tmp = self._expand_and_tile(tf.matmul(A_q, S_inv_ex), 5, 1, M) # N M Q P P
        m = tf.matmul(tmp, self._expand_and_tile(tf.expand_dims(self._subs(Xmu,Z), -1), 5, 2, Q)) # N M Q P 1

        nu_q = tf.expand_dims(self._expand_and_tile(2 * nppi * frequencies, 3, 0, N), -1) # N Q P 1
        nu_q_T = tf.transpose(nu_q, perm=[0,1,3,2]) # N Q 1 P

        exp = self._expand_and_tile(tf.exp(-.5 * tf.matmul(tf.matmul(nu_q_T, A_q), nu_q)), 5, 1, M) # M M Q 1 1
        cos = tf.cos(tf.matmul(self._expand_and_tile(nu_q_T, 5, 1, M), m)) # N M Q 1 1
        fac = weights * (2 * nppi)**(P/2.) * tf.sqrt(Sigma_q_det) # Q
        fac = self._expand_and_tile(self._expand_and_tile(fac, 2, 0, M), 3, 0, N) # N M Q

        # Calculation c: (2 pi)^(-k/2) |det(C)|^(-1/2) exp(-1/2 (x - mu)^T C^{-1} (x - mu))
        mean_c = tf.expand_dims(self._expand_and_tile(self._subs(Xmu,Z), 4, 2, Q), -1)  # N M Q P 1
    ...
    def eKzxKxz(self, Z, Xmu, Xcov):
        ...
        C = self._expand_and_tile(self._expand_and_tile(self._expand_and_tile(C, 3, 0, M), 4, 0, M), 5, 0, N) # N M M Q Q


        # c1
        c1_var = self._expand_and_tile(self._expand_and_tile(self._sum(Sigma_q, Sigma_q), 5, 0, M), 6, 0, M) # M M Q Q P P
        tau = tf.expand_dims(self._expand_and_tile(self._expand_and_tile(self._subs(Z,Z), 4, 2, Q), 5, 2, Q), -1) # M M Q Q P 1
        ...
        # m1
        Sigma_q_inv_ext = self._expand_and_tile(Sigma_q_inv, 4, 0, M) # M Q P P
        y_ext = tf.expand_dims(self._expand_and_tile(Z, 3, 1, Q), -1) # M Q P 1
        tmp = tf.matmul(Sigma_q_inv_ext, y_ext) # M Q P 1
        tmp_ext1 = self._expand_and_tile(self._expand_and_tile(tmp, 5, 0, M), 6, 2, Q) # M M Q Q P 1
        tmp_ext2 = self._expand_and_tile(self._expand_and_tile(tmp, 5, 2, Q), 6, 1, M) # M M Q Q P 1
        m1 = tf.matmul(V1_ext, tmp_ext1 + tmp_ext2) # M M Q Q P 1

        # c2
        S_ext = self._expand_and_tile(self._expand_and_tile(self._expand_and_tile(self._expand_and_tile(Xcov, 4, 1, M), 5, 1, M), 6, 3, Q), 7, 3, Q) # N M M Q Q P P
        c2_var = self._expand_and_tile(V1_ext, 7, 0, N) + S_ext # N M M Q Q P P
        m1_ext = self._expand_and_tile(m1, 7, 0, N) # N M M Q Q P 1
        mu_ext = tf.expand_dims(self._expand_and_tile(self._expand_and_tile(self._expand_and_tile(self._expand_and_tile(Xmu, 3, 1, M), 4, 1, M), 5, 3, Q), 6, 3, Q), -1) # N M M Q Q P 1
        tau = m1_ext - mu_ext
        ...
        # V2
        V1_inv_ext = self._expand_and_tile(self._sum(Sigma_q_inv, Sigma_q_inv), 5, 0, N)  # N Q Q P P
        S_inv = self._expand_and_tile(self._expand_and_tile(tf.matrix_inverse(Xcov), 4, 1, Q), 5, 1, Q)  # N Q Q P P
        V2 = self._expand_and_tile(self._expand_and_tile(tf.matrix_inverse(S_inv + V1_inv_ext), 6, 1, M), 7, 1, M) # N M M Q Q P P

        # m2
        V1_inv_ext = self._expand_and_tile(self._expand_and_tile(self._sum(Sigma_q_inv, Sigma_q_inv), 5, 0, M), 6, 0, M)  # M M Q Q P P
        term1 = self._expand_and_tile(tf.matmul(V1_inv_ext, m1), 7, 0, N) # N M M Q Q P 1
        term2 = tf.matmul(tf.matrix_inverse(Xcov),tf.expand_dims(Xmu,-1)) # N P 1
        term2 = self._expand_and_tile(self._expand_and_tile(self._expand_and_tile(self._expand_and_tile(term2, 4, 1 , Q), 5, 1, Q), 6, 1, M), 7, 1, M) # N M M Q Q P 1
        m2 = tf.matmul(V2,term1 + term2) # N M M Q Q P 1

        # mean_alpha (ma)
        nu_cross = tf.transpose(self._sum(tf.expand_dims(frequencies, -1), tf.expand_dims(frequencies, -1)), perm=[0, 1, 3, 2]) # Q Q 1 P
        nu_cross = self._expand_and_tile(self._expand_and_tile(self._expand_and_tile(nu_cross, 5, 0, M), 6, 0, M), 7, 0, N) # N M M Q Q 1 P
        tmp = tf.matmul(Z,tf.transpose(frequencies)) # M Q
        tmp_ext1 = self._expand_and_tile(self._expand_and_tile(tmp, 3, 0, M), 4, 2, Q) # M M Q Q
        tmp_ext2 = self._expand_and_tile(self._expand_and_tile(tmp, 3, -1, Q), 4, 1, M) # M M Q Q
        tmp_cross_min = -tmp_ext1 - tmp_ext2 # M M Q Q
        ma = tf.squeeze(tf.matmul(nu_cross, m2), axis=[5,6]) + self._expand_and_tile(tmp_cross_min, 5, 0, N) # N M M Q Q

        # mean_beta (mb)
        nu_cross_min = tf.transpose(self._sum(tf.expand_dims(frequencies, -1), tf.expand_dims(-1. * frequencies, -1)), perm=[0, 1, 3, 2]) # Q Q 1 P
        nu_cross_min = self._expand_and_tile(self._expand_and_tile(self._expand_and_tile(nu_cross_min, 5, 0, M), 6, 0, M), 7, 0, N) # N M M Q Q 1 P
        tmp_cross_plus = tmp_ext1 - tmp_ext2 # M M Q Q
        mb = tf.squeeze(tf.matmul(nu_cross_min, m2), axis=[5,6]) + self._expand_and_tile(tmp_cross_plus, 5, 0, N) # N M M Q Q
