...
class TestModelsWithMeanFuncs(unittest.TestCase):
    ...
    def setUp(self):
        ...
        X = rng.randn(self.N, self.input_dim)
        Y = rng.randn(self.N, self.output_dim)
        Z = rng.randn(self.M, self.input_dim)
        self.Xtest = rng.randn(self.Ntest, self.input_dim)
        ...
        self.models_addzero, self.models_multone = \
            [[GPflow.gpr.GPR(X, Y, mean_function=mf, kern=k()),
              ...
              GPflow.sgpmc.SGPMC(X, Y, mean_function=mf, kern=k(), likelihood=GPflow.likelihoods.Gaussian(), Z=Z)]
             for mf in (const, zero, mult, add, addzero, multone)]
